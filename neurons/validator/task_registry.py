...
class TaskRegistry:
    def __init__(
        ...
        - wait_after_first_copy (int): Maximum wait time for the second model after the first acceptable model
            was generated.
    ...
    def complete_task(self, task_id: str, hotkey: str, results: str, score: float) -> None:
        ...
        if task.acceptable_results_time == 0:
            ...
            bt.logging.debug(
                ...
                f"seconds between task started and acceptable results received"
            )
    ...
    def get_task_status(
        self, task_id: str, hotkey: str
    ) -> tuple[Literal["NOT FOUND", "RATE LIMIT", "IN QUEUE", "IN PROGRESS", "DONE", "FAILED"], str | None]:
        task = self._tasks.get(task_id, None)
        if task is None:
