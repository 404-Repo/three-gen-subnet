...
from validator.task_manager.task import GatewayOrganicTask, LegacyOrganicTask, SyntheticTask, ValidatorTask
...
class TestTaskProcessingRules:
    ...
    async def test_send_result_timeout_for_gateway_task(
        ...
        async with get_validator_with_available_organic_tasks(config=config, subtensor=subtensor) as validator:
            ...
            with time_machine.travel(now, tick=True):
                ...
                results = validator.task_manager._organic_task_storage._gateway_manager._gateway_api.results
    ...
    async def test_best_result_for_gateway_task_is_selected(
        ...
        async with get_validator_with_available_organic_tasks(config=config, subtensor=subtensor) as validator:
            ...
            results = validator.task_manager._organic_task_storage._gateway_manager._gateway_api.results
            assert task_id in results
            assert results[task_id].hotkey == max_miner_hotkey
            assert results[task_id].score == max_score
    ...
    async def test_incorrect_data_from_gateway(
        ...
        class Http3ClientMock(Http3Client):
            async def post(self, url: str, data: dict[str, Any]) -> Http3Response:
    ...
    async def test_result_is_submitted_if_timeout_occurs(
        ...
        async with get_validator_with_available_organic_tasks(config=config, subtensor=subtensor) as validator:
            ...
            gateway_result = validator.task_manager._organic_task_storage._gateway_manager._gateway_api.results
