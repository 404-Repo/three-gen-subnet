...
class CLIPCameraProjection(ModelMixin, ConfigMixin):
    ...
    def forward(self, embedding: torch.FloatTensor):
        """
...
class Zero123Pipeline(DiffusionPipeline):
    ...
    def _encode_image(
        ...
        if image_camera_embeddings is None:
            ...
            if isinstance(elevation, float):
                elevation = torch.as_tensor([elevation] * bs_embed, dtype=dtype, device=device)
            if isinstance(azimuth, float):
                azimuth = torch.as_tensor([azimuth] * bs_embed, dtype=dtype, device=device)
            if isinstance(distance, float):
                distance = torch.as_tensor([distance] * bs_embed, dtype=dtype, device=device)
    ...
    def __call__(
        ...
        if isinstance(image, PIL.Image.Image):
            ...
            assert image_latents is not None
            assert clip_image_embeddings is not None or image_camera_embeddings is not None
