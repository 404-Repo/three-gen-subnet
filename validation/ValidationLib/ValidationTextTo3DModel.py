...
import GaussianSplattingRenderer as GSRenderer
import HDF5Loader
...
class ValidateTextTo3DModel:
    def __init__(self, img_width, img_height, views: int = 10, device="cuda"):
        ...
        self.__hdf5_loader = HDF5Loader.HDF5Loader()
        self.__negative_prompts = ['empty', 'nothing', 'false', 'wrong', 'negative', 'not quite right']
    ...
    def init_gaussian_splatting_renderer(self, sh_degree: int = 3, white_background: bool = True, radius: float = 1.0):
        self.__renderer = GSRenderer.GSRenderer(sh_degree, white_background, radius)

    def score_response_gs_input(self, synapses: list[protocol.TextTo3D], cam_rad=1.5, cam_elev=0, save_images: bool = False):
        ...
        for synapse, i in zip(synapses, range(len(synapses))):
            ...
            for azimd in range(0, 360, step):
                ...
                camera = GSRenderer.BasicCamera(pose, self.__img_width, self.__img_height, orbitcam.fovy, orbitcam.fovx, orbitcam.near, orbitcam.far)
