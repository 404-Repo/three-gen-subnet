...
class GaussianProcessor:
    def __init__(self, opt: OmegaConf, prompt: str = ""):
        ...
        self.__W = opt.W
        self.__H = opt.H
        self.__cam = OrbitCamera(
            self.__W, self.__H, r=self.__opt.radius, fovy=self.__opt.fovy
        )
    ...
    def _prepare_training_model(self, models: list):
        ...
        self.__renderer.gaussians.active_sh_degree = (
            self.__renderer.gaussians.max_sh_degree
        )
        ...
        self.__enable_zero123 = (
            self.__opt.lambda_zero123 > 0 and self.__input_image is not None
        )
        ...
        if self.__input_image is not None:
            self.__input_img_torch = (
                torch.from_numpy(self.__input_image)
                .permute(2, 0, 1)
                .unsqueeze(0)
                .to(self.__device)
            )
            ...
            self.__input_mask_torch = (
                torch.from_numpy(self.__input_mask)
                .permute(2, 0, 1)
                .unsqueeze(0)
                .to(self.__device)
            )
        ...
        with torch.no_grad():
            if self.__enable_sd:
                if self.__opt.imagedream:
                    ...
                    self.__guidance_sd.get_text_embeds(
                        [self.__prompt], [self.__negative_prompt]
                    )
    ...
    def _train_step(self):
        ...
        for _ in range(self.__train_steps):
            ...
            if self.__input_img_torch is not None and not self.__opt.imagedream:
                ...
                loss = loss + 10000 * (
                    step_ratio if self.__opt.warmup_rgb_loss else 1
                ) * F.mse_loss(image, self.__input_img_torch)

                # mask loss
                mask = out["alpha"].unsqueeze(0)  # [1, 1, H, W] in [0, 1]
                loss = loss + 1000 * (
                    step_ratio if self.__opt.warmup_rgb_loss else 1
                ) * F.mse_loss(mask, self.__input_mask_torch)
            ...
            for _ in range(self.__opt.batch_size):
                ...
                pose = orbit_camera(
                    self.__opt.elevation + ver, hor, self.__opt.radius + radius
                )
                poses.append(pose)
                ...
                bg_color = torch.tensor(
                    (
                        [1, 1, 1]
                        if np.random.rand() > self.__opt.invert_bg_prob
                        else [0, 0, 0]
                    ),
                    dtype=torch.float32,
                    device="cuda",
                ...
                if self.__enable_sd:
                    if self.__opt.mvdream or self.__opt.imagedream:
                        loss = (
                            loss
                            + self.__opt.lambda_sd
                            * self.__guidance_sd.train_step(
                                images,
                                poses,
                                step_ratio=(
                                    step_ratio if self.__opt.anneal_timestep else None
                                ),
                            )
                        )
                    else:
                ...
                if self.__enable_zero123:
                    loss = (
                        loss
                        + self.__opt.lambda_zero123
                        * self.__guidance_zero123.train_step(
                            images,
                            vers,
                            hors,
                            radii,
                ...
                # densify and prune
                if (self.__step >= self.__opt.density_start_iter) and (
                    self.__step <= self.__opt.density_end_iter
                ):
                    ...
                    self.__renderer.gaussians.add_densification_stats(
    ...
    def train(self, models: list, iters=500):
        if iters > 0:
            ...
            # do a last prune
            self.__renderer.gaussians.prune(
